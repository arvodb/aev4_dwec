...
export class AppComponent {
  ...
  public createValidArr() : string[][]
  {
    ...
    //console.log(this.currentAdjectiveSet)
    for(let i = 0; i < arrLength; i++){
      let newAdj = this.currentAdjectiveSet[this.randomNumber()];
      if(this.numRepitions(output,newAdj) < 1){ //Si no lo encuentra
        console.log(newAdj,i,arrLength);
        output.push(newAdj);
      } else {
        i--;
      }
  ...
  {
    return this.currentTable = this.createValidArr();//Genera un array => transformInTable => devuelve el array en dos dimensiones
  }
